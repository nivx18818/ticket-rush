...
version: alpha
name: Design System
description: A warm, generous visual language anchored on a clean white canvas and a single primary accent color (Rausch — \#ff385c). Type runs Figtree at modest weights — display sits at 22–28px in weight 500/600 rather than the heavy 700+ that fintech and enterprise systems use; the system favors photography and generous whitespace over heavy typography. Three product entries (Homes, Experiences, Services) sit in the top nav with hand-illustrated 32-icon glyphs and "NEW" badges. Pill-shaped search bars (`{rounded.full}`), softly rounded property cards (`{rounded.lg}` ~14px), and 32px button radii read as friendly and human — there are no hard corners in the interactive surface language.
...
This design system defines a generous, photography-led visual language. The base canvas is **pure white** (`{colors.canvas}` — #ffffff) with deep near-black ink (`{colors.ink}` — #222222) for headlines and body, and a single primary accent of **Rausch** (`{colors.primary}` — #ff385c) carrying primary CTAs and accent moments. There is no secondary brand color in mainline marketing — the **Luxe purple** (`{colors.luxe}` — #460479) and **Plus magenta** (`{colors.plus}` — #92174d) tokens are sub-brand accents scoped to Luxe/Plus contexts.

Type runs **Figtree** (a free Google variable font), with **Inter** as the fallback and a system stack underneath. Figtree sits at modest weights — display headlines render at 22–28px in weight 500–600, not the heavy 700+ weights that financial or enterprise systems lean on. The hero h1 ("Inspiration for future getaways") on the homepage is just 28px / 700, which would feel small on a typical SaaS page; here it works because the layout leans on photography (city collage, property cards) for visual weight rather than typographic muscle.
...
- Custom variable type: `Figtree`. Display weights sit at 500–700, body at 400. Modest weight is intentional — the system trusts photography for visual heft.
...
- 8px base spacing system, with major sections at `{spacing.section}` (64px) — generous but not airy enough to feel editorial-magazine; the layout density wants more cards per scroll.

## Colors

### Brand & Accent

- **Rausch** (`{colors.primary}` — #ff385c): The single brand accent. Used for primary CTA backgrounds (Reserve, Continue), the search orb, the heart save state on property cards, and inline brand links.
...
- **Luxe Purple** (`{colors.luxe}` — #460479): Sub-brand accent for Luxe contexts. Only appears inside Luxe-branded surfaces — never in mainline marketing.
- **Plus Magenta** (`{colors.plus}` — #92174d): Sub-brand accent for Plus contexts. Same scoping as Luxe — sub-product only.

### Surface

- **Canvas** (`{colors.canvas}` — #ffffff): The default page floor for every public page. This design system does not include a dark mode for public-facing interfaces.
...
- **Star Rating** (`{colors.star-rating}` — #222222): The same ink token — the star icon and "4.81" rating numbers render in ink rather than a yellow/gold color; this is a deliberate design choice to avoid appearing cheap in travel contexts.
...
The system runs **Figtree** for everything — display, body, navigation, captions, and microcopy. Fallbacks walk `Inter, -apple-system, system-ui, sans-serif`. **Inter** is the fallback.
...
| `{typography.title-sm}`       | 16px | 500    | 1.25        | 0                  | Footer column heads ("Support", "Hosting", "Brand")            |
...
| `{typography.caption-sm}`     | 13px | 400    | 1.23        | 0                  | Footer legal line ("© 2026 Company, Inc.")                     |
...
If Figtree is unavailable, **Inter** is the closest open-source substitute. Adjust display headlines down by ~2% in line-height to match Figtree's slightly tighter cap height; otherwise the proportions transfer cleanly.
...
- **Section padding (vertical):** `{spacing.section}` (64px) for major page bands; tighter than typical SaaS marketing (80–96px) because these pages need higher card density per scroll.
...
- **Footer:** 3-column link list (Support / Hosting / Brand) at desktop, collapsing to 1-column on mobile.

### Whitespace Philosophy

The system gives editorial bands 64px of vertical breathing room but compresses card grids — property and city-link cards sit just 16px apart. The contrast is intentional: the page reads as "open hero, dense card grid below," reinforcing the design intent without overwhelming the visitor at the fold.
...
**`search-orb`** — The circular Rausch orb terminating the right edge of the search bar. 48×48px, fully rounded, white magnifying-glass icon centered. The primary accent moment in the system.

### Top Navigation

**`top-nav`** — White surface, 80px height, 1px bottom hairline. The brand wordmark sits flush left, the three product tabs (Homes / Experiences / Services) sit in the dead center, and account utilities (host link, language globe, account menu) sit flush right.
...
**`footer-light`** — White surface (matches the page canvas — no contrast footer), 48×80px padding. Three columns of link blocks (Support / Hosting / Brand), separated by generous 24px gutters. Each column heads with a `{typography.title-sm}` ink label and stacks `{component.footer-link}` rows in `{typography.body-sm}` ink.
...
- **Hover state colors:** intentionally not documented per the global no-hover policy — the design system's actual `:hover` styling for property cards is a subtle elevation lift, but precise extraction is unreliable.
